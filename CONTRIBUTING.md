...
is no C compiler in the loop - which is what gives you cross-compilation to every
...
- **Mobile (iOS / Android).** Mobile needs JNI / a native app shell / the
  `gomobile` toolchain - i.e. cgo and a C toolchain, the exact things glaze
...
- **No inline `if`-init.** Assign on its own line, then `if`. Not
  `if x, err := f(); err != nil { ... }`.
- **No `else` after a terminal branch** - early-return instead. Prefer `switch`
