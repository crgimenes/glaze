...
// putBounds passes the RECT by value per the target ABI; the implementation is
// arch-specific (see webview2_putbounds_*.go) because a 16-byte struct is passed
// by hidden reference on Win64 amd64 but in two registers on arm64.
...
// addUserScript appends a document-start script and pumps the message loop
// until WebView2 returns its id (matching win32_edge.hh's synchronous wait).
//
// It must NOT be called with w.mu held: it pumps the loop, which can dispatch a
// WebMessageReceived into onMessage() (which locks w.mu). The lock is taken only
// to mutate shared state, then released before pumping.
func (w *webview) addUserScript(src string) {
	...
	w.scriptDone = false
	asWebView2(wv).AddScript(utf16(src), handlerPtr(w.scriptH))
	var m msgStruct
	for !w.scriptDone {
		r := getMessageW(&m, 0, 0, 0)
		if r <= 0 {
...
func (w *webview) Bind(name string, f any) error {
	...
	w.mu.Unlock()
	// Inject a document-start script that binds the name on every load. Done
	// without holding w.mu (addUserScript pumps the message loop).
	w.addUserScript(fmt.Sprintf("(function(){if(window.__webview__){window.__webview__.onBind(%s)}})()", marshalJSON(name)))
